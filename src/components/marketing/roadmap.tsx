...
const RoadmapSection = () => {
  return (
    <div className="bg-transparent min-h-screen text-white flex flex-col items-center py-[150px] px-6">
      <h2 className="text-[64px] font-[700] mb-4">Roadmap</h2>
      <p className="text-center max-w-5xl mb-12 text-white text-[24px] font-[500]">
...
/>
        {roadmapData.map((item, index) => (
          <div key={index} className="flex items-center w-full max-w-2xl mb-8 relative">
            <div style={{background:item.color}} className={`w-6 h-6 rounded-full  absolute left-1/2 transform -translate-x-1/2 z-10 `} />
            <img src={item?.img} className={`absolute left-1/2 transform -translate-x-[11%] z-1  ${item?.transalte}`} />
            

            <div 
            style={{border: "1px solid #2D67FE",background: "linear-gradient(180deg, #2865FF 0%, rgba(40, 101, 255, 0) 100%)"
            }}
              className={`w-[285px] p-4 rounded-lg shadow-lg ${index % 2 === 0 ? "ml-auto " : "mr-auto "}`}
            >
